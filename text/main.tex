...
\begin{enumerate}
    \item \textbf{Data Collection}: Update Tweenvest's code for being able to calculate the scores with the historical data.
    \item \textbf{Data Processing}: Clean and preprocess the collected data to ensure consistency and reliability, handling missing values and outliers appropriately.
    \item \textbf{Descriptive Analysis}: Process and analyze the data to check if the distributions and correlations between variables, looking for early on patterns to later compare and use for the modeling.
    \item \textbf{Algorithms Development}: Develop the a series of predictive models to check what is the best way to use the scores for multiple investment strategies.
    \item \textbf{Backtesting}: Backtest the algorithms to check they are able to generate \textit{alpha}.
\end{enumerate}
