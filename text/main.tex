...
\noindent Also, after this transformation we still didn't see any consistency and statistical significance (see Appendix Table~\ref{tab:regression_results_scores_gaussian}). But since it improved both the performance and the normality \textbf{we applied this transformation for the rest of the models}.
...
\noindent The residues kept having the same issues as the previous models, but we finally see consistency and significance with the value score.
...
\begin{table}[H]
    ...
    \label{tab:regression_results_scores_gaussian}
\end{table}
