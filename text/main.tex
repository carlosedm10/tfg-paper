...
\noindent Examining the results, we observe that a company's 24-month averages for Dividend and Quality are correlated with overall returns, which is consistent with the explained correlations between these scores. The relationship with the Growth score is less intuitive but may still be meaningful: strong growth over the past 24 months suggests stability and should lead to good returns, but if performance in the last 12 months has declined, the market may react negatively. Additionally, there is a clear \textbf{correlation between the most recent Value score and future profits}.
...
\noindent All of these changes made have improved the results so far, but we still are hitting very low performances. This is very common when trying to fit a linear model to a non-linear relationship. But before claiming that that was the cause, we want to see if these non-linearities come from the regions; since the sectors effect should have already been taken into account by Tweenvest's algorithm. Additionally, we wanted to check if the outliers were coming from any specific region.
