...
If we look at the results of the \gls{shap} plots in figures~\ref{fig:small_encoder_shap_consistency} and~\ref{fig:big_encoder_shap_consistency}, we can see that these results are similar to the binary classifier, with less consistency except for the Value scores. This could be due to ability of the model to overfit the data, losing some of the interpretability; but as we have also seen, the model does perform well for unknown stocks so we cannot say it is due to the overfitting.
...
Knowing this, we can see in figures~\ref{fig:regressor_small_binary_shap_consistency} and~\ref{fig:regressor_big_binary_shap_consistency} that also the \acrshort{shap} plots reveal complete noise.
\begin{figure}[H]
    \centering
