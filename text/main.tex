...
\noindent Looking at the performances of the first linear regression models in Figure~\ref{fig:region_performance}, we can appreciate that except for the \textit{North America} region the rest are very low; and the same occurs with the accuracies. Regarding the residuals, there are still generally many outliers and non-normal distributions. However, we especially highlight the extreme values from Asia and Latin America. But even with these results, we start to see some statistical significances in the models' coefficients (see Appendix Figure~\ref{fig:linear_regression_USA_residues} to \ref{fig:linear_regression_AF_residues}).
