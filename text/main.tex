...
That's when we realized that if each method was good at detecting some outliers but bad at others, it makes a lot of sense to use them together. Following this approach and after multiple tests, we found that the bests results came when increasing the overall tolerance and then selecting the intersection of outliers between them, so with a tolerance of 20\% we only excluded around 4\% of the data.
...
\subsection{Summary}
\vspace{0.5cm}
\noindent To summarize how we have treated the data, we ended up having 9 different datasets, each with 17 columns where we have the explained variables, and also the company's name and stock ticker.
