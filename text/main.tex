...
\begin{figure}[H]
    ...
    \caption{\acrshort{multi} Method Schema}
...
When looking at the \acrshort{multi} diagram we can expect it to save different data points that are not common between all of the methods due to its intersection of \textit{true outliers}. This might be good for many classification tasks but when it comes to regression it can cause bias since there could be \textit{"islands"} --clusters-- of points that don't follow a normal distribution. Also since these points don't have to be errors in the data, they could show extreme but consistent behaviour that should be modeled separately.
...
\begin{figure}[H]
    ...
    \caption{Small Data Future - \acrshort{multi} (20\% tolerance)}
